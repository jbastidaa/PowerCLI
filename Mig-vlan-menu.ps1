function Show-Menu
{
    ...
    Write-Host "2:  Cambiar portgroup  VSS a VDS." -ForegroundColor "Green"
    Write-Host "3:  Cambiar portgroup  VDS a VSS." -ForegroundColor "Green"
...
do
{
    ...
    switch ($selection)
    {
        '1' {
            ...
            $portVDS=(Get-Content -Path portVDS.txt)
            $vmsVSS=(get-content -path VSSVM.txt)
            foreach($pVDS in $portVDS){
                foreach($vmVDS in $vmsVSS){
                Get-VM -Name $vmVDS  | Get-NetworkAdapter | Where-Object {$_.NetworkName -eq "STD_$pVDS"} | Set-NetworkAdapter -NetworkName $pVDS -Confirm:$false
                }
            }
            Clear-Variable portVDS
