...
$reporte += foreach($vm in $vms){
                $vm | select name, memoryGB, @{N="TamañoGB";E={(Get-HardDisk -vm $vm | Measure-Object -Sum capacityGB).sum}}, @{N="Espacio total";E={((Get-HardDisk -vm $vm | Measure-Object -Sum capacityGB).sum) + $_.MemoryGB}}
